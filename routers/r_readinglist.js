...
const { checkOnlineMode } = require('../services/serverResponse');

router.get("/",checkOnlineMode,(req,res)=>{
    ...
    lastPage_table.findAll({
        ...
        where:{
            ...
            pageNumber:{
                [Op.gt]:1 // fetch only > 1 pages saved
            }
        },
        limit:12
     }).then((data)=>{
       ...
       res.render("v_comicReadingList",{ data ,  userData: req.session.userData ,searchKeywords:searchKeywords })
       //res.send(data);
    }).catch((err)=>{
        res.send(err);
    })
